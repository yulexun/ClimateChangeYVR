...
The R2 value calculated on the test data ($R^2 = 0.59$) is very close to the R2 value of the model on the training data ($R^2 = 0.61$), as discussed in @sec-result-r2. The $R^2$ value is calculated using the following formula:
...
- $\text{SSR}$ (Sum of Squared Residuals) measures the total squared difference between the observed values ($y_i$) and the predicted values ($\hat{y}_i$).
- $\text{TSS}$ (Total Sum of Squares) measures the total squared difference between the observed values ($y_i$) and their mean ($\bar{y}$).
- $R^2$ quantifies the proportion of the variance in the response variable explained by the model.

 This similarity indicates that the model generalizes well to unseen data, as its performance on the test set is consistent with its performance during training. The small difference between the two R2 values suggests that the model is not overfitting the training data and captures the underlying relationships in the data effectively.


```{r}
#| label: tbl-test-validate
#| tbl-cap: R2 value on test data is close to train data
#| echo: false
#| warning: false
...
results <- data.frame(
  Metric = c("SSR", "TSS", "R2", "R2-Train"),
  Value = c(SSR, TSS, R2, R2T)
)
...
## Predicted vs. Actual Plot shows good overall fit
...
As displayed in @fig-predictedvsfit, points aligning closely along the diagonal line indicate that the predicted log-transformed mean temperature matches well with the observed values. In @fig-predictedvsfit-1, The points closely align along the red diagonal line, indicating that the model predictions are highly accurate for the training dataset. This alignment suggests that the model captures the relationship between the predictors and the log-transformed mean temperature effectively, with minimal bias or variance issues within the training data. In @fig-predictedvsfit-2, the points on the test data plot also align reasonably well along the red diagonal line, though with slightly more scatter compared to the training data. This result indicates that the model generalizes well to unseen data, maintaining good predictive accuracy and avoiding overfitting. The consistency between the train and test plots suggests the model has a good balance between complexity and generalization. In summary, the Predicted vs. Actual plots provide clear visual evidence of the model's accuracy and robustness for both training and test datasets.
