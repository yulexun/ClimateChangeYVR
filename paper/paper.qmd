...
These results highlight the role of atmospheric variables in regional temperature changes and the effectiveness of the polynomial regression approach in capturing complex relationships. Our findings contribute to a deeper understanding of regional climate changes, which is important for decision-making and resilience planning in an important node in global transportation and logistics, where temperature fluctuations directly influence operational efficiency, safety, and infrastructure planning. 
...
The cleaned and combined dataset is split into a training group and a testing group randomly. The training dataset has 70% of the cleaned dataset and the testing dataset has 30% of the cleaned dataset. The training dataset is used to fit the model. The test dataset is used to evaluate the model's performance on unseen data.  
...
# Calculate VIF
vif_values <- round(vif(glm), 2)
kable(vif_values)
```

### Validation on Test Data

We validate our Model LP with test data to evaluate the model's generalizability and performance on unseen data. The result is shown in @tbl-test-validate.  

The R2 value calculated on the test data ($R^2 = 0.59$) is close to the R2 value of the model on the training data ($R^2 = 0.61$), as discussed in @sec-result-r2. This similarity indicates that the model generalizes well to unseen data, as its performance on the test set is consistent with its performance during training. The training MSE of 0.0137 and test MSE of 0.0158 indicate the model performs well on both datasets with minimal overfitting. The small difference between the two R2 values and the small difference between the errors suggests good generalization to unseen data. 
...
results <- data.frame(
  Metric = c("SSR", "TSS", "R2-Test", "R2-Train", "MSE-Train", "MSE-Test"),
...
### Using the Model to Predict Temperature Effects on Airport Operations

The relationship between mean temperature and airport operations, particularly aircraft takeoff performance shows the importance of predictive modeling in adapting to the impacts of climate change. Rising temperatures affect air density, reducing lift and necessitating longer runways or weight restrictions for safe takeoff [@coffel_climate_2015]. These changes not only compromise operational efficiency but also increase costs for airports and airlines. As climate change accelerates, mean temperature fluctuations will become more frequent and extreme, posing greater challenges for airport operations.  

One key application of the model lies in runway management and expansion. Airports facing persistent increases in mean temperature can use predictions from the model to assess the need for longer runways. These expansions would accommodate higher takeoff speeds required under low air density conditions. Similarly, the model's outputs can inform flight scheduling and weight management strategies by identifying periods of elevated temperatures when weight restrictions may be necessary. This allows airports to prioritize lighter-loaded flights, optimize cargo and passenger distribution, and adjust scheduling to minimize disruptions.

The model also supports integration into broader climate resilience strategies. Airports can combine temperature predictions with long-term climate adaptation plans, enabling investments in infrastructure and technology suited for a warming climate. Moreover, by embedding the model into real-time decision support systems, airports can dynamically adjust operations based on updated atmospheric data. 

### Potential Directions for Expanded Analysis

Expanding this research to include airports in different regions around the world would provide a broader understanding of how variables influence airport safety. Airports operate in different climatic and geographic conditions such as coastal locations and high-altitude terrains, and have unique challenges influenced by local weather patterns. Investigating these variations could show patterns and trends that are not observable when focusing on Vancouver's airport. 
